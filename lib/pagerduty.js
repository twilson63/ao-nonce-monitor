...
function buildAggregatedEventPayload(incidents, eventAction, config, options = {}) {
  const payload = {
    routing_key: config.routingKey,
    event_action: eventAction
  };
  ...
  if (eventAction === 'trigger') {
    ...
    const errors = incidents.filter(i => i.error).length;
    const mismatches = totalIncidents - errors;
    
    // Determine overall severity based on worst case
    let overallSeverity = 'warning';
    if (incidents.some(i => {
      if (i.error) return false;
      const diff = Math.abs(parseInt(i.stateNonce) - parseInt(i.suRouterNonce));
      return diff >= 100;
    })) {
      overallSeverity = 'critical';
    } else if (incidents.some(i => {
      if (i.error) return false;
      const diff = Math.abs(parseInt(i.stateNonce) - parseInt(i.suRouterNonce));
      return diff >= 50;
    })) {
    ...
    // Generate summary based on incident types
    let summary;
    if (errors > 0 && mismatches > 0) {
      summary = `${totalIncidents} AO processes affected: ${mismatches} behind scheduler, ${errors} check errors`;
    } else if (errors > 0) {
      summary = `${errors} AO process check errors detected`;
    } else {
      summary = `${mismatches} AO processes behind scheduler`;
    }
    ...
    payload.dedup_key = options.dedupKey || generateDedupKey('aggregated', options.type || 'batch');
    payload.payload = {
      summary: summary,
      ...
      timestamp: new Date().toISOString(),
      custom_details: {
        totalAffected: totalIncidents,
        mismatches: mismatches,
        errors: errors,
        affectedProcesses: incidents.slice(0, 10).map(i => ({
          processId: i.processId,
          stateNonce: i.stateNonce,
          suRouterNonce: i.suRouterNonce,
          slotsBehind: i.error ? null : Math.abs(parseInt(i.stateNonce) - parseInt(i.suRouterNonce)),
          error: i.error || null
        })),
        context: options.context || null,
        monitorType: options.monitorType || 'nonce'
      }
    };
  } else if (eventAction === 'resolve' || eventAction === 'acknowledge') {
    payload.dedup_key = options.dedupKey || generateDedupKey('aggregated', options.type || 'batch');
...
async function sendAggregatedPagerDutyEvent(incidents, eventAction, config, options = {}) {
  if (!config.enabled) {
    return { success: false, error: 'PagerDuty not enabled' };
  }
  
  if (!config.routingKey) {
    console.error(`[${getTimestamp()}] ERROR: PAGERDUTY_ROUTING_KEY not configured`);
    return { success: false, error: 'Routing key not configured' };
  }
  
  if (!incidents || incidents.length === 0) {
    return { success: false, error: 'No incidents provided' };
  }
  
  try {
    const payload = buildAggregatedEventPayload(incidents, eventAction, config, options);
    await postToPagerDuty(payload);
    
    console.log(`[${getTimestamp()}] Aggregated PagerDuty event sent: ${eventAction} for ${incidents.length} incidents`);
    return { success: true, incidentCount: incidents.length, action: eventAction };
  } catch (error) {
    console.error(`[${getTimestamp()}] Failed to send aggregated PagerDuty event: ${error.message}`);
    return { success: false, error: error.message, incidentCount: incidents.length, action: eventAction };
  }
}
...
module.exports = {
  sendPagerDutyEvent,
  sendAggregatedPagerDutyEvent,
  buildEventPayload,
  buildAggregatedEventPayload,
  generateDedupKey,
  loadState,
  saveState,
  getConfigFromEnv
